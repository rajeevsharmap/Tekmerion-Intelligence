...
def evaluate_scammer_hypothesis(evidence: dict, as_of: datetime = None) -> dict:
    ...
    # LLM security boundary: never send raw internal evidence to the
    # external model. `masked_evidence`/`masked_signals` are pseudonymized,
    # deterministic derived copies - see agents/llm_pii_sanitizer.py. The
    # real `evidence`/`derived_signals` objects above are untouched and
    # still returned/used by every other (trusted, internal) caller.
